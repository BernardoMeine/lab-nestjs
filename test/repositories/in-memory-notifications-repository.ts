...
export class InMemoryNotificationsRepository implements NotificationsRepository {
  ...
  async save(notification: Notification): Promise<void> {
    const notifificationIndex = this.notifications.findIndex((item) => item.id === notification.id);

    if (notifificationIndex >= 0) {
      this.notifications[notifificationIndex] = notification;
    }
  }
